...
extern "C" void __hipRegisterFunction(void **modules, const void *hostFunction,
  ...
  fprintf(
      fdebug,
      "modules = %p, hostFunciton = %p, devceFunciton = %s, deviceName = %s\n",
...
void print_result_v2(uint32_t *records, const char *kname, uint32_t num_warps,
                     uint32_t num_branches) {
  fprintf(fdebug, "num_records = %u,%s\n", num_warps * (num_branches + 2),
          kname);
...
#ifdef PRINT_DETAIL
    for (uint32_t branch_index = 0; branch_index < num_branches;
         branch_index++) {
...
extern "C" hipError_t hipLaunchKernel(const void *hostFunction, dim3 gridDim,
  ...
  if (c) {
    ...
    uint32_t size_per_warp =
        (num_branches * 2 * sizeof(uint32_t) +
         2 * sizeof(
                 uint64_t)); // 2 for counters for each branch , 2 for timestamp
    ...
    /*for ( int i =0; i < mempool.size() ; i ++){
      klt_ptr = mempool[i];
      fprintf(fdebug,"Iterating  memory pool at 0x%llx, size =
      %u\n",klt_ptr->data_h, data_size); if(mempool[i]->status == 2 &&
      mempool[i]->data_size >= klt_ptr->avail_size){ klt_ptr = mempool[i]; found
      = true; fprintf(fdebug,"Reusing memory at 0x%llx, size =
      %u\n",klt_ptr->data_h, data_size); break;
      }
      }*/
    hipError_t hip_ret;
    if (found == false) {
    ...
    hip_ret = hipMemcpy(klt_ptr->data, klt_ptr->data_h, data_size,
    ...
    fprintf(fdebug,
            "hip Launch Kernel Called, hostFunction = %p, kernel name = %s\n",
    ...
    *(newArgs + c->kernarg_num + 5) = &(blockDim.y);
    ...
    auto t1 = Clock::now();
    realLaunch(hostFunction, gridDim, blockDim, newArgs, sharedMemBytes,
               stream);
    ...
    hipMemcpy(klt_ptr->data_h, klt_ptr->data, klt_ptr->data_size,
              hipMemcpyDeviceToHost);
    // fprintf(fdata,"%s\n",c->name.c_str());
    // print_result (klt_ptr->data_h, klt_ptr->kid , klt_ptr -> num_records,
    // klt_ptr->num_branches);
    print_result_v2(klt_ptr->data_h, c->name.c_str(), no_warps,
                    klt_ptr->num_branches);
    klt_ptr->status = 2;
...
/*
   ...
   hipMemcpyKind kind){ if(realMemcpy ==0){ realMemcpy = (memcpy_t)
...
__attribute__((constructor)) static void setup(void) {
  ...
  realMemcpy = 0;
  /*if(realRegisterFatBinary == 0){
    realRegisterFatBinary = (registerFatBinary_t)
    dlsym(RTLD_NEXT,"__hipRegisterFatBinary");
    }
...
__attribute__((destructor)) static void fini(void) {
  ...
  for (std::vector<kernel_launch_tracker *>::iterator it = mempool.begin();
       it != mempool.end();) {
    klt = *it;
    if (klt->status == 1) {
      hipFree(klt->data);
